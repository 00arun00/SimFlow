...
def test_dilated_conv_layer_back_prop():
    eps = 1e-7
    ...
    dx_num = numerical_gradient_array(lambda x: dc_layer.forward(x), x, dout, h=eps)
    dw_num = numerical_gradient_array(lambda w: dc_layer.forward(x), w, dout, h=eps)
    db_num = numerical_gradient_array(lambda b: dc_layer.forward(x), b, dout, h=eps)

    out = dc_layer.forward(x)
