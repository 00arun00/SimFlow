...
def test_conv_layer_forward_prop():
    eps = 1e-8
    ...
    c_layer = Conv2D(inChannels=inChannels, outChannels=n_filter,
                     filter_size=filter_size, stride=stride, padding=padding, trainable=True)
    c_layer.W = w
    c_layer.b = b

    out = c_layer.forward(x)
    out_standard = standard_forward(x, w, b, padding, stride)

    print("Testing forward pass of Conv Layer")
    assert np.allclose(out, out_standard, atol=eps)


def test_conv_layer_back_prop():
    eps = 1e-7
    ...
    dx_num = numerical_gradient_array(lambda x: c_layer.forward(x), x, dout, h=eps)
    dw_num = numerical_gradient_array(lambda w: c_layer.forward(x), w, dout, h=eps)
    db_num = numerical_gradient_array(lambda b: c_layer.forward(x), b, dout, h=eps)
