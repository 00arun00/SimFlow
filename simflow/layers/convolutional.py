...
class Conv2D(Layer):
    ...
    def __repr__(self):
        return (
            f"Conv2D Layer with {self.W.shape[0]}"
    ...
    def _convolve_(Input, kernel, bias=0, padding=0, stride=1):
        ...
        Input_col = im2col.im2col_indices(
...
class dilated_Conv2D(Conv2D):
    ...
    def backward(self, dY):
        ...
        dX, _ = self._convolve_(
            Input=dY,
            kernel=dialated_reversed_w.transpose(1, 0, 2, 3),
            padding=dialated_reversed_w.shape[2] - 1,
        )
        assert X.shape == dX.shape

        if self.trainable:
            db = np.sum(dY, axis=(0, 2, 3)).reshape(n_filter, -1)
            dW, _ = self._convolve_(
