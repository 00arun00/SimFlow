...
class minibatch_iterator(Iterator):
    ...
    def __init__(self, batch_size=128, *, shuffle=True):
        ...
        Args:
            ...
            shuffle (bool)   : shuffles each time if set to True
        """
        super(minibatch_iterator, self).__init__(
            batch_size=batch_size, shuffle=shuffle, method="mini_batch"
        )

    def __repr__(self):
        return (
            f"mini_batch iterator "
...
class fullbatch_iterator(Iterator):
    ...
    def __init__(self, *, shuffle=True):
        ...
        Args:
            shuffle (bool)   : shuffles each time if set to True
        """
        super(fullbatch_iterator, self).__init__(
            batch_size=-1, shuffle=shuffle, method="full_batch"
        )
...
class stochastic_iterator(Iterator):
    ...
    def __init__(self, *, shuffle=True):
        ...
        Args:
            shuffle (bool): shuffles each time if set to True
        """
        super(stochastic_iterator, self).__init__(
            batch_size=1, shuffle=shuffle, method="stochastic"
        )
