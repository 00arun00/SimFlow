...
class Model(object):
    ...
    def summary(self):
        ...
        print(
            "{:<30s}|{:22s}|{:20s}".format(
                "Name", "Trainable Parameters", "Total Parameters"
            )
        )
        print(dash)
        total_params = 0
        ...
        for layer in self.layers:
            ...
            print(
                "{:<30s}|{:>22d}|{:>20d}".format(
                    layer.l_name, trainable_param_size, param_size
                )
            )
        ...
        print(
            "{:<30s}|{:>22d}|{:>20d}".format(
                "Total", total_trainable_params, total_params
            )
        )
        print(dash)
    ...
    def set_params(self, params):
        ...
        assert all(
            (
                new_param[0] == old_param[0] and all(n.shape == o.shape)
                for n, o in zip(new_param, old_param)
            )
            for (new_param, old_param) in zip(params, old_params)
        ), "Structure missmatch"
    ...
    def fit(self, Data, Labels, epochs=1, *, verbose=True, **kwargs):
        ...
        for k in kwargs:
            if k not in allowed_kwargs:
                raise TypeError(
                    "Unexpected keyword argument " "passed to fit: " + str(k)
                )
    ...
    def score(self, Data, Labels):
        ...
        """
        assert hasattr(self, "loss"), "please set a loss function to score with"
