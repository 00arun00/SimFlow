...
def test_Dense_forward():
  ...
  d_layer.W = W
  d_layer.b = b
  assert np.allclose(d_layer.forward(Input),standard_forward_Dense(Input,W,b),atol=eps),"forward not working"

def test_linear_back():
    eps = 1e-7
    input_dim = 10
    output_dim = 5
    ...
    dx_num = numerical_gradient_array(lambda x: lin_layer.forward(random_inp), random_inp, random_output_gradient,h=eps)
    dw_num = numerical_gradient_array(lambda W: lin_layer.forward(random_inp), W, random_output_gradient,h=eps)
    db_num = numerical_gradient_array(lambda b: lin_layer.forward(random_inp), b, random_output_gradient,h=eps)


    out = lin_layer.forward(random_inp)
